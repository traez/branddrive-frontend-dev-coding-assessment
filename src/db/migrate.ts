...
export const c3MigrationHistory = pgTable("c3MigrationHistory", {
...
async function ensureMigrationHistoryTable(db: PostgresJsDatabase) {
  console.log("Checking for c3MigrationHistory table...");
  const tableExists = await db.execute(sql`
    SELECT EXISTS (
      SELECT 1 
      FROM information_schema.tables 
      WHERE table_name = 'c3MigrationHistory'
    );
  `);

  if (!tableExists[0].exists) {
    console.log("Creating c3MigrationHistory table...");
    await db.execute(sql`
      CREATE TABLE "c3MigrationHistory" (
    ...
    `);
    console.log("c3MigrationHistory table created successfully.");
  } else {
    console.log("c3MigrationHistory table already exists.");
  }
}
...
async function logMigrationToHistory(
  ...
  const existingMigration = await db
    .select()
    .from(c3MigrationHistory)
    .where(eq(c3MigrationHistory.name, migrationName))
    .execute();

  if (existingMigration.length === 0) {
    console.log(`Logging migration ${migrationName} to c3MigrationHistory...`);
    await db
      .insert(c3MigrationHistory)
...
export async function runMigrations() {
  try {
    ...
    // Log each migration to c3MigrationHistory
